      ...
      <div style={{ padding: "16px" }}>
        {loading ? (
          ...
          <div style={{ ...S.card, textAlign: "center", padding: "60px 20px" }}>
            ...
            <button style={{ ...S.btn, marginTop: "16px" }} onClick={() => setScreen("transfer")}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{
                  ...
                  backgroundColor: t.status === "success" ? `${C.success}22` : `${C.error}22`,
                ...
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: "10px", color: C.gray }}>
                    {new Date(t.timestamp).toLocaleString()}
                  </p>
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                <p style={{ margin: "0 0 4px", fontWeight: "bold", color: C.error }}>
                  -₦{t.amount?.toLocaleString()}
                </p>
                <span style={{
                  fontSize: "10px", padding: "3px 8px", borderRadius: "10px",
                  backgroundColor: t.status === "success" ? "#064e1b"
                    : t.status === "failed" ? "#2d0a0a" : "#1a2a0a",
                  color: t.status === "success" ? C.success
                    : t.status === "failed" ? C.error : C.gold
                }}>
                  {t.status}
                </span>
              </div>
