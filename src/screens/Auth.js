...
export default function Auth({ setScreen }) {
  ...
  const handleAuth = async () => {
    ...
    if (authMode === "register") {
      ...
      if (fullName.trim().split(" ").length < 2) { setAuthError("Please enter both first and last name"); return; }
    }
    if (!phone.trim() || phone.length < 10) { setAuthError("Please enter a valid phone number"); return; }
    if (!authPin || authPin.length !== 4) { setAuthError("PIN must be exactly 4 digits"); return; }
    ...
    try {
      ...
      // Save pin for silent token refresh
      login(data.user, data.access_token, authPin);
      setScreen("dashboard");
    } catch (err) {
  ...
  const handleVerifyOtp = () => {
    if (enteredOtp === generateOtp(existingPhone)) {
      setOtpError(null);
  ...
  const handleResetPin = () => {
    if (newPin.length !== 4) { setOtpError("PIN must be 4 digits"); return; }
      ...
      <div style={{ padding: "24px 20px" }}>
        <div style={{ ...S.card, textAlign: "center", border: `1px solid ${C.gold}44` }}>
          ...
          <button onClick={() => setShowRecovery(true)} style={{
            ...S.btn, backgroundColor: "transparent",
          ...
          <button onClick={resetAll} style={{
            ...S.btn, backgroundColor: "transparent",
            border: "none", color: C.gray, marginTop: "8px"
          }}>
        ...
        {resetSuccess ? (
          <div style={{ ...S.card, textAlign: "center", padding: "40px 20px" }}>
            ...
            <p style={{ color: C.gray, fontSize: "13px", margin: "0 0 20px" }}>
              Your PIN has been reset. Login with your new PIN.
            </p>
          ...
          <div style={S.card}>
            ...
            <h3 style={{ textAlign: "center", margin: "0 0 20px" }}>Set New PIN</h3>
            ...
            <div style={S.numpad}>
              {[1,2,3,4,5,6,7,8,9].map(n => (
                <button key={n} style={S.numKey}
                  onClick={() => newPin.length < 4 && setNewPin(newPin + n)}>{n}</button>
              ))}
              <div></div>
            ...
            <button style={{ ...S.btn, marginTop: "16px", backgroundColor: newPin.length === 4 ? C.orange : C.gray }}
              onClick={handleResetPin} disabled={newPin.length !== 4}>
              Reset PIN
            </button>
          </div>
        ) : otpSent ? (
          <div style={S.card}>
            ...
            <div style={{
              ...
              <p style={{ margin: 0, color: C.gold, fontSize: "12px" }}>
                🧪 Demo: OTP is last 4 digits of your phone number
              </p>
            </div>
            <input style={{ ...S.input, textAlign: "center", fontSize: "24px", letterSpacing: "8px" }}
              placeholder="• • • •" value={enteredOtp}
              onChange={e => setEnteredOtp(e.target.value.replace(/\D/g, "").slice(0, 4))}
              type="tel" maxLength={4} />
            {otpError && <div style={S.error}>❌ {otpError}</div>}
            <button style={{ ...S.btn, backgroundColor: enteredOtp.length === 4 ? C.orange : C.gray }}
              onClick={handleVerifyOtp} disabled={enteredOtp.length !== 4}>
              Verify OTP
            </button>
            <button onClick={handleSendOtp} style={{
              ...S.btn, backgroundColor: "transparent", border: "none", color: C.gray, marginTop: "8px"
            }}>Resend OTP</button>
          </div>
        ) : (
          <div style={S.card}>
            <p style={{ fontSize: "30px", textAlign: "center", margin: "0 0 8px" }}>🔐</p>
            <h3 style={{ textAlign: "center", margin: "0 0 20px" }}>Recover Your Account</h3>
    ...
    <div style={S.wrap}>
      ...
      <div style={{ padding: "24px 20px" }}>
        ...
        <div style={S.card}>
          ...
          <button style={{ ...S.btn, backgroundColor: authLoading ? C.gray : C.orange }}
